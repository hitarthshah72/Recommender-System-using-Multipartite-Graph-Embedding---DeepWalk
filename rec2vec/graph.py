...
def records_to_graph():
    ...
    for user in ratings.keys():
        node = Node(node_id,user,type="user")
    ...
    for movie in movies.keys():
        node = Node(node_id, movie,type="movie")
        nodelist.append(node)
        nodedict[movie] = node
        node_id += 1

        #Creating nodes for movie-ratings
        for rating in range(1,6):
            movie_rating = movie + "_" + str(rating)
            node = Node(node_id, movie_rating, type="rating")
            nodelist.append(node)
            nodedict[movie_rating] = node
            node_id += 1

    #Creating nodes for directors
    for director in directors:
        node = Node(node_id, director, type="director")
        nodelist.append(node)
        nodedict[director] = node
        node_id += 1

    #Creating nodes for actors
    for actor in actors:
        node = Node(node_id, actor, type="actor")
        nodelist.append(node)
        nodedict[actor] = node
        node_id += 1

    #Creating nodes for genres
    for genre in genres:
        node = Node(node_id, genre, type="genre")
        nodelist.append(node)
        nodedict[genre] = node
        node_id += 1
    ...
    for movie in movies.keys():
        ...
        #Adding edges between movies and movie-rating nodes
        for rating in range(1,6):
            movie_rating = movie + "_" + str(rating)
            nodedict[movie_rating].neighbors.append(nodedict[movie])
            nodedict[movie].neighbors.append(nodedict[movie_rating])

        #Adding edges between movies and director nodes
        director = movies[movie].director
        if director:
            nodedict[director].neighbors.append(nodedict[movie])
            nodedict[movie].neighbors.append(nodedict[director])

        #Adding edges between movies and actors nodes
        ...
        #Adding edges between movies and genres nodes
    ...
    # Adding edges between users and movie-ratings nodes
    for user in ratings.keys():
        for movie in ratings[user].keys():
            movie_rating = str(movie) + '_' + str(ratings[user][movie])
            nodedict[movie_rating].neighbors.append(nodedict[user])
            nodedict[user]. neighbors.append(nodedict[movie_rating])
